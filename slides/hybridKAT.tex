...
\{\mathsf{pre}\} &\, \mathsf{therm}\, \{\mathsf{pos}\} &&
...
\begin{itemize}
\item Isabelle/HOL verification components for hybrid programs that
\begin{itemize}
\item benefit from huge libraries of topology, analysis, and ODEs;
\item based on $\MKA$;
\item work with weakest liberal preconditions;
\item support various verification procedures for systems of ODEs, and
\item are correct by construction.
\end{itemize}
\vspace{1em}
...
%\end{itemize}
%\vspace{1em}
\item Yet, program verification requires simpler methods:
\begin{itemize}
\item Hoare logic serves for program verification, and
\item Morgan's refinement calculus is closely related.
\end{itemize}
\end{itemize}
\vspace{\baselineskip}
\begin{center}
\comcol{Can we obtain simpler Hoare logic and refinement calculus to do hybrid program verification?}
\end{center}
\end{frame}
...
Development of minimal proof systems for verification of hybrid systems:
\begin{enumerate}
\item rules of differential Hoare logic $\dH$;
\item laws of differential refinement calculus $\dR$;
\item based on $\KAT$s and $\rKAT$s respectively;
\item integration of lenses as the store model;
\item support reasoning in the style of differential dynamic logic $\dL$, and
...
(\neg p)\, s = &\  
	\begin{cases}
	\{s\}, & \text{ if } p\, s=\emptyset,\\
	\emptyset, & \text{ otherwise},
	\end{cases}\\
\alpha^*\, s = &\ \bigcup_{n\geq 0}\alpha^n\, s,
\end{align*}
where $\alpha^{0}\, s=1\, s$ and $\alpha^{n+1}=\alpha^n\circ_K\alpha$.
\vspace{\baselineskip}
\begin{center}
$\comcol{\hoare{p}{\alpha}{q}\leftrightarrow (\forall s_1.\ p\, s_1 \rightarrow (\forall s_2.\ s_2 \in \alpha\, s_1\rightarrow q\, s))}$
\end{center}
\end{frame}
...
\begin{block}{Lenses}
\begin{itemize}
\item Variables are lenses $x = (A, S, \lget_x, \lput_x)$ where
\begin{equation*}
\mcol{\lget_x:S\to A}\text{ and }\mcol{\lput_x:S\to A\to S}%(denoted $x : A \Longrightarrow S$)
\end{equation*}
\item $A$ provides values of variable while $S$ is the state space
...
%\vspace{\baselineskip}
\begin{block}{}
Semantics $S\to \Pow\, S$ for assignments are
...
\end{align*}
\item Extended to regular programs
\begin{align*}
&\hoare{p}{\sskip}{p},\\
&\hoare{p}{\aabort}{q},\\
...
\item Used modular semantic framework in Isabelle/HOL to
	\begin{itemize}
	\item derive a minimal logic $\dH$ for verification of hybrid programs,
	\item obtain refinement components via the laws of $\dR$,
	\end{itemize}
\item Added lenses for better parsing and alternative program stores
\item Future work:
	\begin{itemize}
	\item Explore total correctness,
	\item Adversarial dynamics like in differential game logic,
	\item Code generation of verified executable code,
