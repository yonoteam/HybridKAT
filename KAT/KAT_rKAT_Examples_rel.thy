...
lemma R_water_tank_inv:
  assumes "0 \<le> \<tau>" and "0 < c\<^sub>o" and "c\<^sub>o < c\<^sub>i"
  shows "rel_R \<lceil>\<lambda>s. Hmin < s$1 \<and> s$1 < Hmax \<and> s$3 = 1\<rceil> \<lceil>\<lambda>s. Hmin \<le> s$1 \<and> s$1 \<le> Hmax\<rceil> \<ge>
  (LOOP 
    \<comment> \<open>control\<close>
    ((2 ::=(\<lambda>s.0));(4 ::=(\<lambda>s. s$1));
    (IF (\<lambda>s. s$3 = 0 \<and> s$4 \<le> Hmin + 1) THEN (3 ::= (\<lambda>s.1)) ELSE 
...
proof-
  ...
  have ifbranch1: "3 ::= (\<lambda>s.1) \<le> rel_R \<lceil>\<lambda>s. ?cond s \<and> ?Icntrl s\<rceil> \<lceil>?Icntrl\<rceil>" (is "_ \<le> ?branch1")
  ...
  hence ctrl: "?ctrl \<le> rel_R \<lceil>?I\<rceil> \<lceil>?Icntrl\<rceil>"
  ...
  DINV (\<lambda>s. s$1 = (c\<^sub>i - c\<^sub>o) \<cdot> s$2 + s$4 \<and> ?dI s)) \<le> rel_R \<lceil>\<lambda>s. s$3 = 0 \<and> ?Icntrl s\<rceil> \<lceil>?I\<rceil>"
  ...
  DINV (\<lambda>s. s$1 = - c\<^sub>o \<cdot> s$2 + s$4 \<and> 0 \<le> s$2 \<and> ?dI s)) \<le> rel_R \<lceil>\<lambda>s. s$3 \<noteq> 0 \<and> ?Icntrl s\<rceil> \<lceil>?I\<rceil>"
    apply(rule R_g_ode_inv)
    using water_tank_diff_inv[OF assms(1), of 0] assms
    by (auto simp: water_tank_inv_arith2)
  have dyn: "?dyn \<le> rel_R \<lceil>?Icntrl\<rceil> \<lceil>?I\<rceil>"
  ...
  have inv_pos: "\<lceil>?I\<rceil> \<le> \<lceil>\<lambda>s. Hmin \<le> s$1 \<and> s$1 \<le> Hmax\<rceil>"
    by simp
  have inv_inv: "rel_R \<lceil>?I\<rceil> \<lceil>?Icntrl\<rceil>; (rel_R \<lceil>?Icntrl\<rceil> \<lceil>?I\<rceil>) \<le> rel_R \<lceil>?I\<rceil> \<lceil>?I\<rceil>"
    by (rule R_comp)
  have loopref: "LOOP rel_R \<lceil>?I\<rceil> \<lceil>?Icntrl\<rceil>; (rel_R \<lceil>?Icntrl\<rceil> \<lceil>?I\<rceil>) INV ?I \<le> ?ref"
    apply(rule R_loop)
    using pre_inv inv_inv inv_pos by auto
  have obs: "?ctrl;?dyn \<le> rel_R \<lceil>?I\<rceil> \<lceil>?Icntrl\<rceil>; (rel_R \<lceil>?Icntrl\<rceil> \<lceil>?I\<rceil>)"
