...
simple program construction and verification examples. We discuss some
of them in detail to explain the different work flows supported by our
...
Isabelle formalisation, is available
...
document that allows reading the Isabelle code without installing the
proof assistant.
...
condition. Such laws can be recursively applied for symbolic
...
Lenses admit several models, including records and
functions~\cite{FosterZW16}. Here, we provide models for
vector spaces, and executable and infinite Euclidean spaces:
%
\begin{align*}
  \textit{vec-lens}^n_k &~\defs~ (\reals, \reals^n, \lambda s.\, \textit{vec-nth}~s~k, \lambda s~v.\, \textit{vec-upd}~k~v~s) & \text{if~} k < n \\
  \textit{eucl-lens}^{(m,n)}_k &~\defs~ (\reals, M^{(m,n)}, \lambda s.\, \textit{eucl-nth}~s~k, \lambda s~v.\, \textit{eucl-upd}~k~v~s) & \text{if~} k < m\!\cdot\!n \\
  \textit{fun-lens}^{(A,B)}_i &~\defs~ (B, A \to B, (\lambda f. f i), (\lambda f~v. \, f(i := v)))
\end{align*}

\noindent The vector lens selects the $k$th element of an
$n$ dimension vector, using $\textit{vec-nth}$ and
$\textit{vec-upd}$, which are developed from the HOL
Analysis library~\cite{HolzlIH13}, which provides a indexed
type for $\reals^n$. The Euclidean lens views executable
Euclidean spaces, developed by Immler~\cite{ImmlerT19}, that
permit both vectors and matrices. Finally, the function lens
selects range elements of a function associated with a
domain element $i \in A$. In particular, it can be used to
describe countably infinite Euclidean spaces,
$\mathbb{N} \to \reals$. All of these lenses satisfy the
three lens axioms~\cite{FosterZW16}.
...
%\ \ \isakeyword{where}\ 
{\isachardoublequoteopen}f\ a\ c\ {\isasymequiv}\ {\isasymlbrakk}{\isacharbrackleft}T\ {\isasymmapsto}\ {\isacharminus}\ a\ {\isacharasterisk}\ {\isacharparenleft}T\ {\isacharminus}\ c{\isacharparenright}{\isacharcomma}\ t\ {\isasymmapsto}\ {\isadigit{1}}{\isacharcomma}\ $T_0$\ {\isasymmapsto}\ {\isadigit{0}}{\isacharcomma}\ {\isasymtheta}\ {\isasymmapsto}\ {\isadigit{0}}{\isacharbrackright}{\isasymrbrakk}\isactrlsub e{\isachardoublequoteclose}\isanewline
\end{isabellebody} 
...
\noindent The first line uses tactic \isa{hyb-hoare} to blast away the structure of \isa{therm} using $\dH$. To apply \isa{hyb-hoare}, the program must be an iteration of the composition of two programs --usually control and dynamics. The tactic requires a description of the store between those programs via an Isabelle UTP-predicate, indicated by the function $U(-)$. Lemma \isa{local-flow-therm},
...
%\ \ \isakeyword{where}\ 
{\isachardoublequoteopen}f\ k\ {\isasymequiv}\ {\isasymlbrakk}{\isacharbrackleft}h\ {\isasymmapsto}\ k{\isacharcomma}\ t\ {\isasymmapsto}\ {\isadigit{1}}{\isacharcomma}\ $h_0$\ {\isasymmapsto}\ {\isadigit{0}}{\isacharcomma}\ {\isasympi}\ {\isasymmapsto}\ {\isadigit{0}}{\isacharbrackright}{\isasymrbrakk}\isactrlsub e{\isachardoublequoteclose}\isanewline
\isanewline
\isacommand{abbreviation}\ %\isamarkupfalse%
...
itself---and this time a differential invariant \isa{dI} as well:
\begin{isabellebody}
\isanewline
\isacommand{abbreviation}\ %\isamarkupfalse%
...
\noindent As in Example~\ref{ex:therm-sol}, tactic \isa{hyb-hoare} blasts away
the control structure of this program. The second line of the proof
uses lemma \isa{tank-diff-inv} to check that \isa{dI} is an invariant,
...
obligations are atomic refinements. We only refine those atomic
programs that supply crucial information to complete proofs
automatically. For \isa{therm}, these were those for the first two
assignments and the differential equations shown above. \qed
\end{example}

\begin{example}[Water tank refinement via invariants]\label{ex:tank-rinv}
  Alternatively we can use the differential invariant to refine
  \isa{tank{\isacharunderscore}dinv} from Example~\ref{ex:tank-inv}
  using $\dR$. This time we supply a single structured proof to show
  another style of refinement. We %omit some parts of the proof and
...
\ \ \ LOOP\ {\isacharparenleft}{\isacharparenleft}t\ {\isacharcolon}{\isacharcolon}{\isacharequal}\ {\isadigit{0}}{\isacharparenright}{\isacharsemicolon}\isactrlbold {\isacharbrackleft}I\ h\isactrlsub l\ h\isactrlsub h\ {\isasymand}\ t\ {\isacharequal}\ {\isadigit{0}}{\isacharcomma}\ I\ h\isactrlsub l\ h\isactrlsub h\isactrlbold {\isacharbrackright}{\isacharparenright}\ INV\ I\ h\isactrlsub l\ h\isactrlsub h{\isachardoublequoteclose}\ {\isacharparenleft}\isakeyword{is}\ {\isachardoublequoteopen}{\isacharunderscore}\ {\isasymge}\ {\isacharquery}R{\isachardoublequoteclose}{\isacharparenright}\isanewline
\ \ \ \ \isacommand{by}\isamarkupfalse%
...
\ \ \ {\isacharparenleft}{\isacharparenleft}t\ {\isacharcolon}{\isacharcolon}{\isacharequal}\ {\isadigit{0}}{\isacharparenright}{\isacharsemicolon}{\isacharparenleft}h\isactrlsub {\isadigit{0}}\ {\isacharcolon}{\isacharcolon}{\isacharequal}\ h{\isacharparenright}{\isacharsemicolon}\isactrlbold {\isacharbrackleft}I\ h\isactrlsub l\ h\isactrlsub h\ {\isasymand}\ t\ {\isacharequal}\ {\isadigit{0}}\ {\isasymand}\ h\isactrlsub {\isadigit{0}}\ {\isacharequal}\ h{\isacharcomma}\ I\ h\isactrlsub l\ h\isactrlsub h\isactrlbold {\isacharbrackright}{\isacharparenright}\ INV\ I\ h\isactrlsub l\ h\isactrlsub h{\isachardoublequoteclose}\ {\isacharparenleft}\isakeyword{is}\ {\isachardoublequoteopen}{\isacharunderscore}\ {\isasymge}\ {\isacharquery}R{\isachardoublequoteclose}{\isacharparenright}\isanewline
\ \ \ \ \isacommand{by}\isamarkupfalse%
\ {\isacharparenleft}refinement{\isacharcomma}\ rel{\isacharunderscore}auto{\isacharprime}{\isacharparenright}\isanewline
\ \ \isacommand{moreover}\isamarkupfalse%
\ \isacommand{have}\isamarkupfalse%
...
The proof incrementally refines the program structure of
\isa{tank{\isacharunderscore}dinv} by applying the laws of $\dR$. As
in Example~\ref{ex:therm-rsol}, after refining the first two
assignments, the \isa{refinement} tactic completes the refinement of
\isa{ctrl}. Then, the invariant is provided via lemma
\isa{tank{\isacharunderscore}diff{\isacharunderscore}inv} from
Example~\ref{ex:tank-inv} to refine \isa{dyn}. The main result is
obtained by transitivity of $\leq$. With a proof assistant, the user
owns the means of production. A more fine-grained program construction
is equally possible. \qed
\end{example}
...
calculi~\cite{LiuLQZZZZ10}, or integrate CAS's like Mathematica for
finding solutions with minimal effort. It should be equally
straightforward to formalise $\dH$ and $\dR$ based on other Hoare
logics in Isabelle by plugging in our hybrid store models.
