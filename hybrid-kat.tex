...
composition and use a guard $G$ to restrict evolutions between $T_l$ and
$T_h$ by setting
\begin{equation*}
G\, T_l\, T_h\, a\, c = \left(t\leq -\frac{1}{a}\ln\left(\frac{c-\Delta_c}{c-T_0}\right)\right),
\end{equation*}
where $\Delta_c = T_l$ if $c=0$, and $\Delta_c = T_h$ if $c=T_u$. Below, the hybrid program $\isa{therm}\ T_l\ T_h\ a\ T_u$ models the thermostat's behaviour. We separate it in loop invariant, discrete control and continuous dynamics.
...
\isa{therm} with the rules of $\dH$. With lemma \isa{local{\isacharunderscore}flow{\isacharunderscore}therm}, whose proof required the procedure described above, the second line supplies the flow for $f\, a\, c$. That is,
\begin{equation*}
...
automatically with auxiliary lemmas. \qed
\end{example}
