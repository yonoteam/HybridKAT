...
\jin{Perhaps the reviewer was expecting a standard presentation of logics $\dH$ and $\dR$ where syntax, grammar, semantics are introduced one after the other. However, that approach would be unfaithful to the development and discovery of the logics as extensions of $\KAT$}
...
programs. 
\rin{Could we do the same things without lenses? They don't seem essential to your development, i.e., the power of lenses seems overkill for what you're trying to do.}
\jin{Below we indicate how we take advantage of the power of lenses.}
...
\rin{Another question that came up, is whether $n = tn$ or $n=n^3$ is the defining requirement here for antitests. I don't really see the antitests in the remainder of the paper either.}
...
\jin{Answers to all these questions are given below or can be deduced from the results}
...
$\alpha^\ast$, we obtain
\rin{For (h-inv-plus), consider changing the left hand side $\wedge$ into a $\vee$.}
...
\rin{Here, the notion of independence needs to be explained (or at least make explicit that you are not referring to linear independence)} Each program variable is a lens
...
\rin{For terms 'trajectory' and 'integral curve', use the word 'the' to disambiguate which of these terms apply to whether the term applies if it is uniquely defined.}
\jin{Changed it.}
...
\jin{Reviewer is right. The sentence above (st-evl-flow), should state that $U$  can only be a closed interval $[0,t]$ for $t\in T$, just like in the Isabelle formalisation.}
...
\rin{What does the bold U mean? Does it bind T?}
\jin{Idem, and explained below.}
...
\rin{what is Guard? (Is it a free variable, syntax or a constant?)}
