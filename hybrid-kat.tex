...
of an element $\alpha\in K$ as an element $i\in B$ satisfying $\{i\}\,
\alpha\, \{i\}$. 
...
generalise both $\dL$-style differential invariants~\cite{Platzer18}
...
The following Morgan-style refinement laws~\cite{Morgan94} of a
\emph{propositional refinement calculus} are derivable in
...
For invariants and loops, we can easily derive the refinement laws
\begin{align}
...
compositionally in the style of Morgan. To our knowledge this is the
first refinement calculus for hybrid programs of this kind.  A more
...
Finally, we present a verification component that starts directly from
...
\end{equation*}
with respect to the guarded orbit of $\flow_s$ along $T$ in $s$. It is
then straightforward to derive a Hoare triple and basic refinement law
for such evolution commands along the lines of previous sections.
...
semigroups~\cite{Struth18}. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\gin{we need to cite this paper with refinement~\cite{LoosP16} ---
  quite incredible that they got a lics paper out of it!
}
