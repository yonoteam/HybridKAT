...
\begin{example}[Thermostat verification via solutions]\label{ex:therm-sol}
  ...
  $T_m\le T\le T_M$. Variable $T_0$ stores an initial
  temperature;  $\theta$ indicates wether the heater is switched
  on or off.  At most every $\tau$ minutes, the thermostat resets time to
  $0$, measures the temperature, and turns the heater on or off
  accordingly. The following hybrid program models 
  its behaviour, separated in discrete control and continuous
  dynamics.
...
automatically when
supplying auxiliary lemmas to Isabelle. \qed
\end{example}
...
\begin{example}[Water tank verification via invariants]\label{ex:tank-inv}
  ...
  Variable $h_0$ indicates water-level measurements \gin{what does
    this mean?}  and \isa{Pump} \gin{Write $\pi$ for pump?}  whether
...
This time we reason with invariants. Hence our correctness
...
program structure. We thus omit this part of the proof until an
application of (\ref{eq:h-cond}) splits the proof obligation into one
for each evolution command. The result is discharged using rule
...
\begin{example}[Water tank refinement via invariants]\label{ex:tank-rinv}
  ...
  Example~\ref{ex:tank-inv}. This tim we supply a single structured
