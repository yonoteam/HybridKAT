...
Lenses admit several models, including records and total functions~\cite{FosterZW16}. Here, we provide model for vector
...
\noindent The vector lens selects the $k$th element of an $n$ dimension vector, using $\textit{vec-nth}$ and
$\textit{vec-upd}$, which are developed from the HOL Analysis library~\cite{HolzlIH13}. The Euclidean lens views
executable Euclidean spaces, developed by Immler~\cite{ImmlerT19}, that permit both vectors and matrices. Finally, the
function lens selects range elements of a function associated with a domain element $i \in A$. In particular, it can be
used to describe countably infinite Euclidean spaces, $\mathbb{N} \to \reals$. All of these lenses satisfy the three
lens axioms~\cite{FosterZW16}.
