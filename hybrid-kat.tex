...
differentiable are equal if they intersect at some point and their
derivatives are equal. Therefore, the rule (\ref{eq:h-invg}) and
Propositions~\ref{P:inv-prop} and~\ref{P:invrules} yield the following
procedure for proving the triple $\{P\}\, x' = f\, \&\, G\, \{Q\}$
using an invariant.
\begin{enumerate}
\item Check whether a candidate predicate $I$ is a differential invariant:
	...
	\item for atomic $I$, apply Lemma~\ref{P:invrules} (1) and (2);
	\end{enumerate}
...
For invariants and loops, we can derive, in addition, 
\begin{align}
...
Finally, we can derive the following refinement law for evolution
commands.
...
Finally, we introduce a verification component that starts directly
from flows $\flow:T\to S\to S$ instead of vector fields.  This avoids
checking the conditions of the Picard-Lindel\"of theorem.  A syntax
for hybrid programs would now be
\begin{equation*}
\mathcal{C}\ ::= \ x:=e \mid \mathbf{evol}\, \flow\, G \mid ?P\mid \mathcal{C};\mathcal{C}\mid \mathcal{C}+\mathcal{C}\mid \mathcal{C}^*
\end{equation*}
although we do not use it with our shallow embedding. Instead, the
flow-based evolution command is simply a semantic illusion for the
guarded orbit of $\flow_s$ along $T$ in $s$,
\begin{equation*}
  (\mathbf{evol}\, \flow\, G) = \lambda s.\ \gamma^{\flow_s}_G\, s.
\end{equation*}
It is then straightforward to derive a Hoare triple and basic
...
\begin{example}[Bouncing ball via Hoare logic]\label{ex:ball-hoare}
  Suppose a ball of mass $m$ falls down from height $h\geq 0$, which $x$ denoting its position
  and $v$ its velocity. Its kinematics is modelled by the vector field
...
$s_x = s\, x$ and $s_v = s\, v$.  The ball bounces back
...
\begin{equation*}
\{P\}\, \mathsf{Ball}\, \{Q\}\qquad\text{ for }\quad
...
\gin{we need to cite this phd thesis with refinement}
