...
operator for state updates, $\sigma \dagger e = e \circ s$,
\gin{$\sigma$?} that simply composes expression $e$ with $\sigma$. We
can then define a semantic analog of the substitution operator,
...
\begin{align*}
  x := x &~=~ \sskip, \\
  x := e \seq x := f &~=~ x := f[e/x], \\
  x := e \seq y := f &~=~ y := f \seq x := e, \qquad\qquad \textnormal{if~} x \lindep y, x \unrest f, y \unrest e, \\[.5ex]
...
\begin{example}[Thermostat verification via solutions]\label{ex:therm-sol}
  A thermostat regulates a room's temperature between $T_m$ and
  $T_M$. It uses variable $T_0$ to store an initial temperature and
  $\theta$ to indicate wether the heater is switched on or
  off. Variable $T$ denotes the room's temperature and $t$ is time. At
  most every $\tau$ minutes, the thermostat resets time to
  $0$, measures the room's temperature, and turns the heater on or
  off accordingly. An Isabelle formalisation of a
  hybrid program to model this behaviour, separated in discrete
  control and continuous dynamics, is

\gin{replace $\Theta$ by $\theta$.}
...
$U={\isacharbraceleft}{\isadigit{0}}{\isachardot}{\isachardot}{\isasymtau}{\isacharbraceright}=[0,\tau]$. For
specification purposes we write $;$ instead of $\cdot$ for sequential
...
\end{equation*}
where $\Delta_c = T_m$ if $c=0$, and $\Delta_c = T_M$ if $c=L$. We define the vector fields via
...
The rule applications blast away the program structure of
\isa{therm}. The fifth line, in particular, simplifies the proof
obligation with (\ref{eq:h-cond}) and (\ref{eq:h-evl}). This is
possible because lemma
\isa{H{\isacharunderscore}g{\isacharunderscore}ode{\isacharunderscore}therm}
uses the fact that the flow for $f\, a\, c$ is
\begin{equation*}
...
inequalities of exponentials and logarithms. They are discharged by
supplying auxiliary lemmas to Isabelle's \isa{auto} method. \qed
\end{example}
...
\begin{example}[Water tank verification via invariants]\label{ex:tank-inv}
  ...
  water $h$ in a tank within a safe region $h_m\leq h\leq h_M$. As in
  Example~\ref{ex:therm-sol}, we use $t$ for time, $h_0$ for
  water-level measurements and \isa{Pump} for indicating whether the
  pump is on or off. The rate of change for the water-level in the
  tank changes linearly with slope $k\in\{-c_o,c_i-c_o\}$ (assuming
  $c_i>c_o$). The vector field mapping that represents this behaviour
  is
\begin{isabellebody}
\isanewline
\isacommand{abbreviation}\isamarkupfalse%
...
For this example, we use the invariant annotated version of evolution
commands, which means that our correctness
specification for \isa{tank{\isacharunderscore}dinv\ c\isactrlsub i\
  c\isactrlsub o\ h\isactrlsub m\ h\isactrlsub M\ {\isasymtau}}  and
its verification is 
\begin{isabellebody}
\isanewline
\isacommand{lemma}\isamarkupfalse%
...
As in Example~\ref{ex:therm-sol}, the rule applications decompose the
program structure of the hybrid program. We thus omit this part
of the proof unitl an application of (\ref{eq:h-cond})
splits the proof obligation into one for each evolution
command. The result is discharged using rule (\ref{eq:h-invg}) followed by
the fact that $dI\, h_m\, h_M\, k$ is an invariant for $f\, k$
(\isa{tank{\isacharunderscore}diff{\isacharunderscore}inv}) and some
arithmetical properties. \qed
\end{example}
...
Here we refine \isa{therm} from Example~\ref{ex:therm-sol} in a modular way. Proving
\begin{equation*}
...
\end{equation*} 
amounts to showing that the body of the loop also refines the
specification. For this we prove separate refinement statements for the
control and dynamics, which we then compose. This implies
ensuring that the loop invariant $I$ is a precondition of the control
and a postcondition of the dynamics and that any postcondition of the
control will be the precondition of the dynamics.
\begin{isabellebody}
\isanewline
\isacommand{lemma}\isamarkupfalse%
...
In particular, for the dynamics, we use (\ref{eq:r-evl}) to prove that
...
specification. We use this fact and transitivity to show our main
goal.
...
then a combination of the previous lemmas and the laws of our
refinement calculus.\qed
\end{example}
...
\begin{example}[Water tank refinement via invariants]\label{ex:tank-rinv}
  ...
  Example~\ref{ex:tank-inv}. Instead of doing it modularly, we supply
  a single structured proof. We omit uncritical parts using ellipses
...
refinement version, the proof for the control is longer as we
