...
sets~\cite{Teschl12} that describe evolutions implicitly. Verification
...
take to extend a standard Hoare logic or refinement calculus to
hybrid programs.
...
verification components, we mainly need to formalise a state
transformer semantics for $\KAT$ and $\rKAT$ along the lines
of~\cite{afp:hybrid} and a concrete store semantics for hybrid
...
Our Isabelle components support the usual workflows of $\dL$ in $\dH$
and $\dL$: we may reason explicitly with solutions to differential
equations and implicitly with invariant sets. We have formalised third
method in which solutions, that is flows, are declared ab initio in
correctness specifications and need not be certified.

Our verification components have so far be evaluated on a small set of
...
flows within our approach. With Isabelle tactics for automated
...
\tau\in {\downarrow}t.\ G\, (\flow_s\, \tau)\right) \rightarrow P\,
(\flow_s\, t)\right\}\, x' = f\, \&\, G\, \{P\}. \label{eq:h-evl}\tag{h-evl}
\end{equation}
\end{lemma}
...
\begin{align}
 (x := e)  &\le  \left[\lambda s.\ P\, s[x\mapsto e\, s],P\right],\label{eq:r-assgn}\tag{r-assgn}\\
(x:= e) \cdot \left[\lambda s.\ P\, s[x\mapsto e\, s],P\right] &\le [P,P],\label{eq:r-assgn}\tag{r-assgnl}\\
\left[P,\lambda s.\ P\, s[x\mapsto e\, s]\right]\cdot (x:=e) &\le [P,P]. \label{eq:r-assgn}\tag{r-assgnf}
\end{align}
...
This article contributes new methods and Isabelle components to a
modular and open semantic framework for verifying hybrid systems that
so far focussed on predicate transformer methods~\cite{MuniveS19};
more specifically
\begin{itemize}
...
mathematical theory supported by Isabelle, it should also allow
...
plugging in our hybrid store model.
...
forever---or even based on multirelational
semantics~\cite{FurusawaS16,FurusawaS15} as needed for differential
game logic~\cite{Platzer15}. This, however, is beyond the scope of
this article.
