...
Isabelle~\cite{afp:kat}.  As type classes allow only one type parameter, we
use an alternative to the standard two-sorted approach that expands a Kleene algebra $K$ by an
...
have been developed specifically for this article. By Proposition~\ref{P:kleisli-ka},all rules derived in this section hold in particular in the state transformer semantics. We have formalised this fact with Isabelle. At this stage, verification condition rules for the basic commands for assignments and evolution commands are still missing. These are added in the concrete state transformer semantics as explained in Section~\ref{sec:hoare-flow}.
...
of $\KAT$---function $?(-)$ embeds tests explicitly into programs; in the tradition of $\KAT$ we leave this embedding implicit.
...
\rin{$(1)$ Here, the notion of independence needs to be explained (or at least make explicit that you are not referring to linear independence)} 
\jin{For Simon to address.} Each program variable is a lens
...
$\reals^n$. Yet more general state spaces such as vector spaces are supported as well.
...
\end{equation*}
where ${\downarrow}t = \left\{t'\in T\mid t'\le t\right\}$. Intuitively, $\gamma_G^X\, s$ is the orbit at $s$ defined along the longest interval in $T$ that satisfies $G$. We also define the
...
In the evolution command $x'= f\, \&\, G$, $x'$ is part of the traditional syntax used for specifying systems of ODEs, while in reality only a vector field $f$ is specified. This explains why $x$ does not appear in the right-hand side of~(\ref{eq:st-evl}). Defining the state transformer semantics of assignments is
...
In the state transformer semantics of Hoare triples, the Kleisli composition in
...
after its execution. Specifically for evolution commands, and consistently with
$\dL$, $q$ holds along the entire orbit of a solution for $f$. We now complete the derivation of inference rules of $\dH$ by adding Hoare-style rules for assignments and evolution commands in the concrete state transformer semantics.
...
interval $[0,t]$ for some $t\in T$~\cite{MuniveS19} where, by contrast to the previous section ${\downarrow}t=\{t'\in U\mid t'\leq t\}$ is now relativised to $U$. This relativisation seems important for the analysis of hybrid systems. First of all, it gives the flexibility to consider dynamics over closed time intervals, secondly it allows us to focus on time intervals and IVPs starting at time $t=0$. Accordingly,
(\ref{eq:st-evl}) specialises to the following state transformer semantics for evolution commands.
\begin{equation}
...
so that the recursive application of the rules of propositional Hoare logic together with (\ref{eq:h-assgn}) and (\ref{eq:h-evl}) generates proof obligations that are
...
\noindent In the Isabelle code above, we distinguish DINV and INV to make specifications intelligible.The correctness specification and verification of the water
tank with $\dH$ is the following.
...
invariant for any guard (that is, \isa{Guard} is a universally quantified variable in Lemma \isa{tank-diff-inv}), using the procedure outlined (see our repository for
...
and increased proof automation via the tactics \isa{hyb-hoare} and \isa{refinement}. 
...
refinements between hybrid programs. According to the authors~\cite{LoosP16}, it differs substantially from the
calculi developed by Back and von Wright~\cite{BackW98},
Morgan~\cite{Morgan94} and others in that local instead of global refinement relations can be used. On the other hand however, their refinement logic has the same expressivity as $\dL$~\cite{LoosP16}, which is essentially a predicate transformer calculus for hybrid programs~\cite{MuniveS19} and therefore, a Back a von Wright style refinement calculus. Ultimately, this suggests that Loos' refinement logic is more expressive than our refinement calculus. The relative merits of these approaches remain to be explored. The proof theory of $\dL$ has already been formalised in various proof assistants~\cite{BohrerRVVP17} yet with a focus on soundness proofs for its inference rules and a mechanisation of its idiosyncratic substitution calculus in a deep embedding, but not on verification components.
...
The authors wish to thank the reviewers for their very thorough and insightful comments, and to Andr\'e Platzer and Sergey Goncharov for discussions of a preliminary version. The second author is sponsored by CONACYT's scholarship no. 440404.
