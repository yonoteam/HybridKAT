...
developing these  methods requires simply adding a single
...
assignment and evolution commands. The axioms and laws for these
...
which a \emph{test} function $t:K\to K$ is defined as $t=n\circ n$. Then
$K_t = \{\alpha \mid t\, \alpha = \alpha\}$ forms a boolean algebra in
which $n$ yields test complementation. It can be used in place of
$B$.  The state transformer $\KAT$ has been formalised for
this article.
...
$s$, parametrised by $T$ and tangential to $f$ at any point in $S$; it
...
\end{equation*}
Each solution $X$ is thus continuously differentiable and hence
...
traditional syntax used for specifying systems of ODEs, while de facto
only a vector field $f$ is specified. This explains why $x$ does not
appear in the right-hand side of~(\ref{eq:st-evl}). Defining the state
transformer semantics of assignments is standard~\cite{MuniveS19},
though we generalise using lenses. First, we use lenses to define
state updates:
...
compatible with the invariants from
...
\begin{lemma}[\cite{MuniveS19}]\label{P:invrules}
  ...
  differentiable and $T\subseteq \reals$ an interval such that $0\in T$.%\footnote{An \emph{interval} is any set $T$ satisfying that $\forall a\in T.\forall b\in T. \forall x.\ a\leq x\leq b\to x\in T$.}.
\begin{enumerate}
\item If $(\mu\circ X)' =(\nu\circ X)'$ for all
...
have the same derivatives. Rules (\ref{eq:h-invg}),
...
\begin{enumerate}
\item Check whether  candidate predicate $I$ is an invariant for $f$
  along $T$:
	\begin{enumerate}
...
\begin{example}[Water tank verification via invariants]\label{ex:tank-inv}
  ...
  $h$ in a tank within bounds $h_l\leq h\leq h_h$.  Variable
...
\isanewline
\end{isabellebody}
\noindent  Program \isa{tank-dinv} for the controller is
given by guard $G\ h_x\ k$ with
$h_x\in\{h_l,h_h\}$ that restricts evolutions beyond $h_x$,
loop invariant $I$, control and dynamics:
\begin{isabellebody}
\isanewline
\isacommand{abbreviation}\ 
...
\noindent Tactic \isa{hyb-hoare} blasts away the control structure. The second proof
line uses Lemma \isa{tank-diff-inv} to check that \isa{dI} is an
invariant for any guard (\isa{Guard} is a universally
...
yet applications remain to be explored.  A previous approach to refinement
in $\dL$~\cite{LoosP16} is quite different to the two standard calculi
mentioned (see Conclusion).
...
\isa{ctrl}. After that, the invariant is supplied via lemma
...
\begin{example}[Bouncing ball via Hoare logic and refinement]\label{ex:ball-hoare}
  A ball  falls down from height $h\geq 0$, with
...
refinements between hybrid programs. According to the authors, it
differs substantially from the standard
approaches~\cite{BackW98,Morgan94} in that local instead of global
refinement relations can be used. Nevertheless their refinement logic
has the same expressivity as $\dL$~\cite{LoosP16}, which is
essentially a predicate transformer calculus for hybrid
programs~\cite{MuniveS19} and thus a refinement calculus
\`a la Back and von Wright. Ultimately, this suggests that Loos' logic
is more expressive than our Morgan-style calculus, but the relative
merits of the two approaches remain to be explored. The proof theory
of $\dL$ has already been deeply embedded in proof
assistants~\cite{BohrerRVVP17}, yet with a focus on soundness proofs
for its inference rules and a mechanisation of its idiosyncratic
substitution calculus, but not as prima facie verification components.
...
semantics~\cite{FurusawaS16,FurusawaS15}, which are relevant needed
for differential game logic~\cite{Platzer18}. This is left for future
work.

\paragraph{Acknowledgements.}
The authors wish to thank the reviewers for their very thorough and
insightful comments, and to Sergey Goncharov and Andr\'e Platzer for
discussions on a preliminary version. The second author is sponsored
by CONACYT's scholarship no. 440404; the first author is supported by
the EPSRC project CyPhyAssure\footnote{\url{https://www.cs.york.ac.uk/circus/CyPhyAssure/}} (Grant
EP/S001190/1).
