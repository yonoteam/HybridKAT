...
1+\alpha\cdot\alpha^\ast &\le \alpha^\ast, \qquad
  ...
  1+\alpha^\ast\cdot\alpha &\le \alpha^\ast, \qquad \gamma+\beta\cdot
                             ...
                             \alpha^\ast \le \beta.
\end{align*}
In these axioms, the ordering $\le$ on $K$ defined by
...
$f^{\ast}\, x = \bigcup_{i\in\mathbb{N}} f^i\, x$ are defined with
  ...
  $f^{(i+1)} = f \circ_K f^{n}$. The boolean algebra of tests has
...
approach that expands an Kleene algebra $K$ by an \emph{antitest}
...
$\alpha\ \mathbf{inv}\ i$ can easily be derived (operationally,
...
specific for loops of hybrid programs---see
...
reasoning with general invariants has been generated for this
article. 
...
an autonomous system ordinary differential equations (ODEs) and a
guard $G$, a predicate modelling boundary conditions or similar
restrictions of temporal evolutions. Guards are also known as
\emph{evolution domain restrictions}~\cite{DoyenFPP18}.

Formally, vector fields are functions of type $S\to S$ on some open
$S\subseteq \reals^n$ for some $n\in\mathbb{N}$---the state space of
...
in vector form---and $X\, 0 = s$. $X$ is thus a curve in $S$ through
$s$, parametrised in $T$ and tangential to $f$ at any point in $S$.
If $X$ is uniquely defined, then it is called \emph{trajectory} or
\emph{integral curve} of $f$ at $s$.
...
$\gamma^X_G:S\to \Pow\, S$ as \begin{equation*} \gamma^X_{G}\, s =
...
{\downarrow}t.\ G\, (X\, \tau)\}.
\end{equation*}
\gin{shall we only/always use predicates?}  Expanding definitions,
...
\begin{equation}
  (v:= e) = \{f_a\, v\, e\}.\label{eq:st-assgn}\tag{st-assgn}
\end{equation}
...
orbitals, he state transformer semantics is arguably conceptually
...
Next we derive a rule for evolution commands.

\gin{adapt to lipschitz-continuous}

We restrict our attention to Lipschitz-continuous vector fields for which
unique solutions to IVPs are guaranteed by the Picard-Lindel\"of
theorem.  Solutions then arise from \emph{(local)\ flows}
$\flow:T\to S\to S$ and $X=\flow_s=\lambda t.\ \flow\, s\, t$ is the
trajectory at $s$. We can then specialise $\gamma^f_G$ to
\begin{equation*}
...
For invariants and loops, we obtain
\begin{align}
...
Int $\sta\, S$, moreover, the following laws for assignments are
derivable~\cite{ArmstrongGS16}.
\begin{align*}
 (x := e)  &\le  [\lambda s.\ P\, s[x\mapsto e\, s],P],\\
(x:= e) \cdot [\lambda s.\ P\, s[x\mapsto e\, s],P] &\le [P,P],\\
[P,\lambda s.\ P\, s[x\mapsto e\, s]]\cdot (x:=e) &\le [P,P].
\end{align*}
The second and third law are known as \emph{leading} and {following}
law. They introduce an assignment before and after a block of code. 

\gin{derive law for evolution statements ... from that of Hoare
  logic.}
